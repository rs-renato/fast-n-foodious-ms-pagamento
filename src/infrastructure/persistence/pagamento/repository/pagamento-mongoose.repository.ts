...
export class PagamentoMongoDbRepository implements IRepository<Pagamento> {
  ...
  async createPagamento(createPagamentoDto: CreatePagamentoDto): Promise<PagamentoDocument> {
    const createdPagamento = new this.pagamentoModel(createPagamentoDto);
    return createdPagamento.save();
  }

  async updatePagamento(pagamentoId: number, updatePagamentoDto: UpdatePagamentoDto): Promise<PagamentoDocument> {
